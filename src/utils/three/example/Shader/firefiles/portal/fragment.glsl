...
*/
float cnoise(vec3 p){
    vec3 pi0 = floor(p); // 索引的整型值
    vec3 pi1 = pi0 + vec3(1.); // 索引的整型值 + 1
    pi0 = mod(pi0 ,289.0);
    pi1 = mod(pi1 ,289.0);
    ...
    vec4 n_z = mix(vec4(n000,n100,n010,n110),vec4(n001,n010,n011,n111),fade_xyz.z);
...
void main(){
    // 给UV 添加柏林噪声，UV 坐标扩大了7倍
    vec2 displacedUv = vUv + cnoise(vec3(vUv * 7.,uTime * 0.1));
