    ...
    return new THREE.ShaderMaterial({
        ...
        fragmentShader:`
            ...
            void main(){
                ...
                float theta = acos(nPos.y ) / PI;// double acos(double x),x范围在 -1~1 之间
                float phi = 0.;
                phi = (atan(nPos.z,nPos.x) + PI) / (2. * PI); // 圆360°
                phi += 0.75;
                vec4 pColor = texture2D(uPanorama,vec2(phi,theta));

                gl_FragColor = pColor;

                if(nPos.z < 0.003 && nPos.z > - 0.003 && nPos.x < 0.){
                    phi = (atan(0.003,nPos.x) + PI) / (2. * PI);
                    phi += 0.75;
