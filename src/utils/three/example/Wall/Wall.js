    ...
    init(){
        ...
        faceIndexs.forEach((item)=>{
            ...
            let isFace = this.faceRelation.some((face)=>{
                faceItem = face;
                return (item.includes(face.index[0]) && item.includes(face.index[1]) && item.includes(face.index[2]) && item.includes(face.index[3]));
            });
        ...
        faces = faceIndexs.map((item,index)=>{
            return [
        ...
        for(let i = 0; i < faces.length;i++){
            ...
            uvs.push(...faceUvs);
            indices.push(...faceIndexs);
            normals.push(...faceNormals[i],...faceNormals[i],...faceNormals[i],...faceNormals[i]);

            //设置材质组
            materialGroups.push({
                start:i * 6,count:6,materialIndex:i
            });
        }
