...
vec4 taylorInvSqrt(vec4 r){
    return 1.79284291400159 - 0.853734720953124 * r;
}
...
void main(){
    ...
    float xPart = vUv.x * 1000. / resolution.x; // 扩大了1000倍
    float yPart = vUv.y * 1000. / resolution.y;

    float clip = 210.;

    float fireStrength = 1. - uFrequenct / 100.;
    ...
    float xFuel  = 1. - abs(2. * xPart - 1.); // or pow(1. - abs(2. * xPart - 1.),0.5);
    float timespeed = 0.5;
    float realTime = timespeed * time;

    vec2 coordScaled = 0.01 * gl_FragCoord.xy - 0.02 * vec2(offset.x,0.);
    ...
    if(sparkLife > 0){
        float sparkSize = xFuel * sFuel * sparkRandom * 0.08;
