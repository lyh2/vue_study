...
export class BiLuFire {
  ...
  createFire() {
    ...
    this.fireGeometry = new THREE.CylinderGeometry(
      0.03,
      0.2,
      0.35,
      15,
  ...
  createAshes() {
    ...
    this.ashesGeometry = new THREE.CylinderGeometry(
      0.15,
      0.15,
      0.4,
      15,
  ...
  createGUI() {
    ...
    this.gui
      .add(this.fireMaterial.uniforms.stylizeThreshold, 'value', 0, 1)
  ...
  updateLight() {
    ...
    const g =
      Math.abs(Math.sin(this.time + Math.PI / 2) + Math.cos(this.time * 4 + 1.4) * 0.5) * 0.2;
...
export class DrawPolygonUseEarcut {
    ...
    if (this.points?.length > 0) {
      ...
      if (this.points.length > 1) {
        for (let i = 0; i < tempPositionArray.length - 1; i++) {
          this.createLine(
            tempPositionArray[i],
            tempPositionArray[i + 1],
            true,
            tempPositionArray,
            this.polygons.length,
            tempPositionArray[tempPositionArray.length - 1]
          );
        }
      }
    }
  ...
  createLine(pointBegin, pointEnd, closedLoop = false, inputPositions, polygonIndex, point) {
    ...
    if (closedLoop && inputPositions.length > 2) {
      ...
      const flatVertices = [
        -36, 11, 1, 32, 28, 0.129, -0.16, -22, -0.5, 0.5, 0.5, 0.5, 0.5, -0.5, -0.5, -0.5,
      ];
      const flatVertices_ = [
        -36, 0.1, 11, 1, 0.1, 32, 28, 0.1, 0.129, -0.16, 0.1, -22, -0.5, 0.1, 0.5, 0.5, 0.1, 0.5,
  ...
  findMiddlePoint(inputPositions) {
    ...
    const sum = inputPositions.reduce(
...
export class DrawPolygonOfPoints3D {
  ...
  convexHull() {
    ...
    normal
      .crossVectors(
        this.points3D[1].clone().sub(this.points3D[0]),
        this.points3D[2].clone().sub(this.points3D[0])
      )
      .normalize();
    ...
    function cross(o, a, b) {
      return (
        (a.projected.x - o.projected.x) * (b.projected.y - o.projected.y) -
        (a.projected.y - o.projected.y) * (b.projected.x - o.projected.x)
      );
    }
    ...
    for (let i = projected.length - 1; i >= 0; i--) {
      ...
      while (
        upperHull.length >= 2 &&
        cross(upperHull[upperHull.length - 2], upperHull[upperHull.length - 1], p) <= 0
      ) {
        upperHull.pop();
      }
      upperHull.push(p);
...
class Portal {
  ...
  set effectMultiplier(v) {
    ...
    new TWEEN.Tween(this.plane.material.uniforms.effectMultiplier)
      .to(v, 1)
...
class World extends EventEmitter {
  ...
  moveWorldToEnd() {
    ...
    new TWEEN.Tween(this.holder.position)
      .to(this.endPosition, duration)
      .easing(TWEEN.Easing.Linear.In);
    new TWEEN.Tween(this.holder.scale)
      .to(this.holder.endScale, duration)
      .easing(TWEEN.Easing.Quartic.In);
    new TWEEN.Tween(this.holder.quaternion)
      .to(this.endQuaternion, duration)
  ...
  moveWorldAndCameraToOrigin() {
    ...
    new TWEEN.Tween(this.holder.scale)
      .to(new THREE.Vector3(1, 1, 1), duration)
      .easing(TWEEN.Easing.Quintic.In);
    new TWEEN.Tween(this.holder.quaternion)
      .to(new THREE.Quaternion(0, 0, 0, 1), duration)
      .easing(TWEEN.Easing.Quintic.In);

    new TWEEN.Tween(this.cameraTarget.position)
      .to(new THREE.Vector3(0, 0, 0), duration)
      .easing(TWEEN.Easing.Cubic.In);
    new TWEEN.Tween(this.perspectiveCamera.position)
      .to(new THREE.Vector3(0, 0, 40), duration)
      .easing(TWEEN.Easing.Cubic.In)
      .onUpdate(() => {
  ...
  moveCameraToPortal() {
    ...
    new TWEEN.Tween(this.cameraTarget.position)
      .to(this.portalWorldEnd.position, duration)
...
class App {
  ...
  createControls() {
    this.controls = new OrbitControls(
      this.currentWorld.perspectiveCamera,
...
export class UseGeometryUtils {
  ...
  init() {
    ...
    const line = new THREE.Line(
      geometrySpline,
      new THREE.LineDashedMaterial({ color: 0xffffff, dashSize: 2, gapSize: 0.5 })
    );
    ...
    const tempParametricGeometry = new ParametricGeometry(klein, 25, 25);
  ...
  createOuterBigBall() {
    ...
    instancedGeometry.setAttribute(
      'aPosition',
      new THREE.InstancedBufferAttribute(aPositionFloat32, 3)
    );
  ...
  startTimeline() {
    let bg1 =
      'linear-gradient(#633b73, #211b20) padding-box, linear-gradient(#211b20, #633b73) border-box';
    let bg2 =
      'linear-gradient(#6e5b4f, #302313) padding-box, linear-gradient(#302313, #6e5b4f) border-box';
...
export class FireWork {
  ...
  init() {
    ...
    // 创建曲线
    const tempCurve3 = new THREE.CubicBezierCurve3(
      new THREE.Vector3(-30, 0, 0),
      new THREE.Vector3(-15, 30, -20),
      new THREE.Vector3(15, 45, 25),
...
export class PenEffect {
  ...
  createGUI() {
    ...
    this.gui.add(floorSimMatUniforms.waterQuantity, 'value', 0.0, 1).name('Water Quantity');
...
export class Rain {
  ...
  animateWaterDropPosition() {
    ...
    this.interval = setInterval(() => {
      ...
      if (this.pause) {
        ...
        new TWEEN.Tween(waterDrop.position)
          .to(new THREE.Vector3(x, 0, z), 1)
          .onUpdate(val => {
  ...
  getRandomWaterDropPosition() {
    return this.waterDropPositions[
      Math.floor(Math.random() * Math.floor(this.waterDropPositions.length))
    ];
  }
  createGrid() {
      ...
      for (let col = 0; col < this.grid.cols; col++) {
        ...
        for (let r = 0; r < this.ripples.length; r++) {
          ...
          if (dist < ripple.radius) {
            ...
            const y = this.reDefineMap(
              Math.sin(angle),
              -1,
              0,
              ripple.motion > 0 ? 0 : ripple.motion,
              0
            );
            pivot.position.y = y;
          }
  ...
  createObjects() {
    ...
    for (let i = 0; i < 200; i++) {
      this.createBox(
        ...
        2 + Math.random() * 4,
        Math.random() + 1,
        new THREE.Vector3((0.5 - Math.random()) * 50, 5, 50 * (0.5 - Math.random())),
        120,
        0xb0b0b0
      );
    }
  ...
  initInput() {
    ...
    this.renderer.domElement.addEventListener('pointerdown', evt => {
      ...
      ballBody.velocity.set(
        this.raycaster.ray.direction.x * 24,
  ...
  breakeObject(mesh, body, impactPoint, impactNormal) {
    ...
    for (const fragment of debris) {
      if (index < debris.length) {
        ...
        this.scene.add(fragment);
        fragment.position.set(Math.random() * 50, Math.random() * 5, Math.random() * 50);
        const merged = BufferGeometryUtils.mergeVertices(fragment.geometry);
...
export class ShaderSmoke {
  ...
  animate() {
    this.renderer.render(this.scene, this.perspectiveCamera);
    if (this.particles) this.particles.material.uniforms.uTime.value += this.clock.getDelta();
  }
  _windowResizeFun() {
    this.perspectiveCamera.aspect = window.innerWidth / window.innerHeight;
    this.perspectiveCamera.updateProjectionMatrix();
...
class Firework {
  ...
  init() {
    ...
    geometry.setAttribute(
      'instT',
      new THREE.InstancedBufferAttribute(new Float32Array(pos), 3, false)
    );
    geometry.setAttribute(
      'instR',
      new THREE.InstancedBufferAttribute(new Float32Array(rot), 4, false)
    );
    geometry.setAttribute(
      'instS',
      new THREE.InstancedBufferAttribute(new Float32Array(scl), 3, false)
    );
    geometry.setAttribute(
      'instDelay',
      new THREE.InstancedBufferAttribute(new Float32Array(delay), 1, false)
    );
  ...
  addRocket() {
    ...
    const rocket = new Rocket({
      ...
      position: {
        ...
        y: THREE.MathUtils.randFloat(6, 15),
...
const getPointMesh = (num, vels, type) => {
  ...
  bufferGeometry.setAttribute(
    'adjustSize',
    new THREE.BufferAttribute(new Float32Array(adjustSizes), 1)
  );
...
export class FireworkParticle {
  ...
  makeRoughGround() {
    ...
    for (let i = 0; i < position.count; i++) {
      const noise1 =
        this.noise.noise3d(
          position.array[i].x * 0.01 + time * 0.0002,
          position.array[i + 1].y * 0.01 + time * 0.0002,
          position.array[i + 2].z * 0.01 + time * 0.0002
        ) * 5;
      const noise2 =
        this.noise.noise3d(
          position.array[i].x * 0.02 + time * 0.00002,
          position.array[i + 1].y * 0.02 + time * 0.00004,
          position.array[i + 2].z * 0.02 + time * 0.00002
        ) * 2;
      const noise3 =
        this.noise.noise3d(
          position.array[i].x * 0.015 + time * 0.00003,
          position.array[i + 1].y * 0.012 + time * 0.00003,
          position.array[i + 2].z * 0.015 + time * 0.00003
        ) * 2;
...
export class MouseNoise {
  ...
  init() {
    ...
    this.rtTexture_2 = new THREE.WebGLRenderTarget(
