...
function start() {
    ...
    if (Rotation) {
        ...
        document.getElementById("鏃嬭浆").src = "assets/texture/xuanzhuan.gif";
        document.getElementById("1").style.background = '#fff35d';
        document.getElementById("鏃嬭浆鏂囧瓧").innerText = "鍋滄鏃嬭浆";
        document.getElementById("鏃嬭浆鏂囧瓧").style.color = "#3c3c3c";
        help.style.display = 'none';

    } else {
        orbitControls.autoRotate = false;
        document.getElementById("鏃嬭浆").src = "assets/texture/xuanzhuan_off.png";
        document.getElementById("1").style.background = '#444444';
        document.getElementById("鏃嬭浆鏂囧瓧").innerText = "鑷姩鏃嬭浆";
        document.getElementById("鏃嬭浆鏂囧瓧").style.color = "#cccccc";
...
function init() {
    ...
    gltfLoader.load('2F_wall.gltf', function (gltf) {
        gltf.scene.traverse(function (child) {
            if (child.isMesh) {
                ...
                child.material.transparent = true;
                child.material.alphaTest = 0.1;
                //computeFaceNormals鏂规硶閲嶆柊璁＄畻涓夎闈㈠璞＄殑娉曠嚎鍚戦噺,璁＄畻娉曠嚎鍚戦噺锛屽奖鍝嶇殑鏄痜ace鏁扮粍涓瘡涓厓绱犵殑normal灞炴€э紝涓€涓猣ace鍙湁1涓�
                child.geometry.computeFaceNormals();
                //computeVertexNormals鏂规硶閲嶆柊璁＄畻涓夎闈㈠璞￠《鐐圭殑娉曠嚎鍚戦噺,face鏁扮粍涓瘡涓厓绱犵殑vertexNormal灞炴€э紝涓€涓猣ace3鍨嬪璞℃湁3涓紝涓€涓猣ace4鍨嬪璞℃湁4涓紝
                // 浣嗘槸闇€瑕佹敞鎰忕殑鏄紝琚涓〃闈㈠叡浜殑椤剁偣锛屽叾娉曠嚎鍚戦噺鍙湁涓€涓紝鍚屾椂鍙楀埌澶氫釜琛ㄩ潰鐨勫奖鍝嶃€傛瘮濡備腑蹇冨湪鍘熺偣锛屼笁缁勮〃闈㈤兘鍨傜洿浜庤酱鐨勭珛鏂逛綋锛�
                // 鍏剁涓€璞￠檺涓殑椤剁偣锛屾硶绾垮悜閲忔槸(1,1,1)鐨勫綊涓€鍖栥€傝櫧鐒剁湅涓婂幓涓嶅彲鎬濊锛屽钩闈㈢殑椤剁偣鐨勬硶绾垮眳鐒朵笉鏄瀭鐩翠簬骞抽潰鐨勶紝浣嗚繖绉嶆寚瀹氭硶绾跨殑鏂规硶
                // 鍦ㄥ埄鐢ㄥ钩闈㈡ā鎷熸洸闈㈢殑鏃跺€欐湁寰堝ソ鐨勬晥鏋溿€�
                child.geometry.computeVertexNormals();
                child.geometry.computeBoundingBox();
                child.geometry.computeBoundingSphere();
                //emissive鐗╀綋鍙嶅皠鐨勯鑹�,榛樿涓洪粦鑹�
    ...
    gltfLoader.load('3F_main.gltf', function (gltf) {
        gltf.scene.traverse(function (child) {
            if (child.isMesh) {
                child.material.side = THREE.DoubleSide;
                child.material.transparent = true;
                //鏀瑰彉涓嶅悓鐨刟lpha娴嬭瘯鍊硷紝浠ラ€傚悎鍦烘櫙,鍒濇瑙ｅ喅浜嗛€忔槑璐村浘鏄剧ず闂
                child.material.alphaTest = 0.1;
                //computeFaceNormals鏂规硶閲嶆柊璁＄畻涓夎闈㈠璞＄殑娉曠嚎鍚戦噺,璁＄畻娉曠嚎鍚戦噺锛屽奖鍝嶇殑鏄痜ace鏁扮粍涓瘡涓厓绱犵殑normal灞炴€э紝涓€涓猣ace鍙湁1涓�
                child.geometry.computeFaceNormals();
                //computeVertexNormals鏂规硶閲嶆柊璁＄畻涓夎闈㈠璞￠《鐐圭殑娉曠嚎鍚戦噺,face鏁扮粍涓瘡涓厓绱犵殑vertexNormal灞炴€э紝涓€涓猣ace3鍨嬪璞℃湁3涓紝涓€涓猣ace4鍨嬪璞℃湁4涓紝
    ...
    function switchsanwei() {
        if (camera instanceof THREE.OrthographicCamera) {
            ...
            document.getElementById("涓夌淮鏂囧瓧").innerText = "骞抽潰鍥�";
            document.getElementById("涓夌淮鏂囧瓧").style.color = "#cccccc";
            ...
            document.getElementById("涓夌淮鏂囧瓧").innerText = "3D楦熺灠";
            document.getElementById("涓夌淮鏂囧瓧").style.color = "#3c3c3c";
    ...
    function Floor2() {
        ...
        qiangtiswitch = false;
        document.getElementById("闅愯棌澧欎綋").src = "assets/texture/wall_off.png";
        document.getElementById("3").style.background = '#fff35d';
        document.getElementById("澧欎綋鏂囧瓧").innerText = "鏄剧ず澧欎綋";
        document.getElementById("澧欎綋鏂囧瓧").style.color = "#444444";
    ...
    function all() {
        ...
        document.getElementById("澧欎綋鏂囧瓧").innerText = "闅愯棌澧欎綋";
        document.getElementById("澧欎綋鏂囧瓧").style.color = "#cccccc";
    }
    ALL = all;
    ...
    function Qiangti() {
        qiangtiswitch = !qiangtiswitch;
        if (qiangtiswitch) {
            document.getElementById("闅愯棌澧欎綋").src = "assets/texture/wall_on.png";
            document.getElementById("3").style.background = '#444444';
            document.getElementById("澧欎綋鏂囧瓧").innerText = "闅愯棌澧欎綋";
            document.getElementById("澧欎綋鏂囧瓧").style.color = "#cccccc";
        } else {
    ...
    function UI() {
        ...
        if (UIswitch) {
            ...
            document.getElementById("妤煎眰鏂囧瓧").style.color = "#ffffff";
            ...
            document.getElementById("妤煎眰鏂囧瓧").style.color = "#cccccc";
        }
    }
    loucengUI = UI;
    ...
    /* function onDocumentMouseDown1(event) {
         ...
         if (intersects.length > 0) {
             ...
             document.getElementById("鍏ㄦ櫙閫€鍑�").style.display = "";
