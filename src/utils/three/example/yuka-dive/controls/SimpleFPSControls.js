  ...
  update(delta) {
    ...
    if (this.pitchUp || this.pitchDown) {
      this.pitch.rotation.x -=
        ((Number(this.pitchDown) - Number(this.pitchUp)) * this.lookSpeedY) / 1000;
      this.pitch.rotation.x = Math.max(-PI_2, Math.min(PI_2, this.pitch.rotation.x));
    }

    if (this.yawLeft || this.yawRight) {
      this.yaw.rotation.y -=
        ((Number(this.yawRight) - Number(this.yawLeft)) * this.lookSpeedX) / 1000;
    }
    ...
    if (this.moveUp || this.moveDown) {
      ...
      this.object.position.y = Math.max(
        this.minY,
        Math.min(this.maxY, this.object.position.y - y_contrib)
      );
    }
  }
}
