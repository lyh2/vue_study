...
export default class World {
  ...
  _initLevel() {
    ...
    // 进行空间划分，可提高查询判断效率
    this.navMesh.spatialIndex = new YUKA.CellSpacePartitioning(
      width,
      height,
      depth,
      cellsX,
    ...
    if (this.debug) {
      ...
      // 创建可视化对象表示产生敌人的点位
      this.helpers.spawnHelpers = SceneUtils.createSpawnPointHelper(
        this.spawningManager.spawningPoints
      );
  ...
  _initEnemies() {
    ...
    for (let i = 0; i < enemyCount; i++) {
      const name = 'Enemy:' + i;
      const renderComponent = SceneUtils.cloneWithSkinning(
        this.assetManager.modelMaps.get('soldier')
      ); // 这里为啥不用clone(),简单的clone() 不能正确复制骨骼、蒙皮等数据，需要自己单独实现或者调用three.js 中的SkeletonUtils.clone() 方法
      renderComponent.name = name;
      const enemy = new EnemyVehicle(this /* world */);
      //const enemy =new  Enemy(this /* world */);
      enemy.name = name;
  ...
  checkProjectileIntersection(projectile, intersectionPoint) {
    ...
    for (let i = 0; i < entities.length; i++) {
      ...
      if (
        entity !== owner /* 不与自己进行碰撞测试 */ &&
        entity.active &&
        entity.checkProjectileIntersection /* 对象是否存在某个方法 */
      ) {
