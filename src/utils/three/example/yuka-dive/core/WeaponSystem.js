...
export default class WeaponSystem {
      ...
      switch (weapon.type) {
        case WEAPON_TYPES_BLASTER:
          ...
          if (this.owner.isPlayer)
            weapon.setRenderComponent(
              this.renderComponents.blaster.mesh,
              this.owner.world.sync.bind(this.owner.world)
            );
          break;
        case WEAPON_TYPES_SHOTGUN:
          this.renderComponents.blaster.mesh.visible = false;
          ...
          if (this.owner.isPlayer)
            weapon.setRenderComponent(
              this.renderComponents.shotgun.mesh,
              this.owner.world.sync.bind(this.owner.world)
            );
          break;
        case WEAPON_TYPES_ASSAULT_RIFLE:
          ...
          if (this.owner.isPlayer)
            weapon.setRenderComponent(
              this.renderComponents.assaultRifle.mesh,
              this.owner.world.sync.bind(this.owner.world)
            );
          break;
        default:
    ...
    if (target) {
      ...
      if (targetSystem.isTargetShootable()) {
        ...
        if (owner.haveAttacker) {
          // 表示已经有子弹击中了owner，这个时候我应该旋转面向敌人，进行还击
  ...
  reload() {
    ...
    if (
      currentWeapon.status === WEAPON_STATUS_READY ||
  ...
  _initShotgunFuzzyModule(fuzzySets) {
    ...
    fuzzyModuleShotGun.addRule(
      new YUKA.FuzzyRule(new YUKA.FuzzyAND(fuzzySets.targetClose, lowShot), fuzzySets.desirable)
    ); // 距离很近+剩余子弹数量很少 = 理想的，可接受的
    fuzzyModuleShotGun.addRule(
      new YUKA.FuzzyRule(
        new YUKA.FuzzyAND(fuzzySets.targetClose, okayShot),
        fuzzySets.veryDesirable
      )
    ); //
    fuzzyModuleShotGun.addRule(
      new YUKA.FuzzyRule(
        new YUKA.FuzzyAND(fuzzySets.targetClose, loadsShot),
        fuzzySets.veryDesirable
      )
    );

    fuzzyModuleShotGun.addRule(
      new YUKA.FuzzyRule(new YUKA.FuzzyAND(fuzzySets.targetMedium, lowShot), fuzzySets.undesirable)
    ); // 距离远+剩余子弹数量少 = 不理想的，不可接受的
    fuzzyModuleShotGun.addRule(
      new YUKA.FuzzyRule(new YUKA.FuzzyAND(fuzzySets.targetMedium, okayShot), fuzzySets.undesirable)
    );
    fuzzyModuleShotGun.addRule(
      new YUKA.FuzzyRule(new YUKA.FuzzyAND(fuzzySets.targetMedium, loadsShot), fuzzySets.desirable)
    );

    fuzzyModuleShotGun.addRule(
      new YUKA.FuzzyRule(new YUKA.FuzzyAND(fuzzySets.targetFar, lowShot), fuzzySets.undesirable)
    ); // 距离远 + 剩余子弹少 = 不理想的，不可接受的
    fuzzyModuleShotGun.addRule(
      new YUKA.FuzzyRule(new YUKA.FuzzyAND(fuzzySets.targetFar, okayShot), fuzzySets.undesirable)
    );
    fuzzyModuleShotGun.addRule(
      new YUKA.FuzzyRule(new YUKA.FuzzyAND(fuzzySets.targetFar, loadsShot), fuzzySets.undesirable)
    );
  ...
  _initBlasterFuzzyModule(fuzzySets) {
    ...
    // 添加规则
    fuzzyModuleBlaster.addRule(
      new YUKA.FuzzyRule(
        new YUKA.FuzzyAND(fuzzySets.targetClose, lowBlaster),
        fuzzySets.undesirable
      )
    );
    fuzzyModuleBlaster.addRule(
      new YUKA.FuzzyRule(new YUKA.FuzzyAND(fuzzySets.targetClose, okayBlaster), fuzzySets.desirable)
    );
    fuzzyModuleBlaster.addRule(
      new YUKA.FuzzyRule(
        new YUKA.FuzzyAND(fuzzySets.targetClose, loadsBlaster),
        fuzzySets.desirable
      )
    );

    fuzzyModuleBlaster.addRule(
      new YUKA.FuzzyRule(new YUKA.FuzzyAND(fuzzySets.targetMedium, lowBlaster), fuzzySets.desirable)
    );
    fuzzyModuleBlaster.addRule(
      new YUKA.FuzzyRule(
        new YUKA.FuzzyAND(fuzzySets.targetMedium, okayBlaster),
        fuzzySets.desirable
      )
    );
    fuzzyModuleBlaster.addRule(
      new YUKA.FuzzyRule(
        new YUKA.FuzzyAND(fuzzySets.targetMedium, loadsBlaster),
        fuzzySets.desirable
      )
    );

    fuzzyModuleBlaster.addRule(
      new YUKA.FuzzyRule(new YUKA.FuzzyAND(fuzzySets.targetFar, lowBlaster), fuzzySets.desirable)
    );
    fuzzyModuleBlaster.addRule(
      new YUKA.FuzzyRule(new YUKA.FuzzyAND(fuzzySets.targetFar, okayBlaster), fuzzySets.desirable)
    );
    fuzzyModuleBlaster.addRule(
      new YUKA.FuzzyRule(new YUKA.FuzzyAND(fuzzySets.targetFar, loadsBlaster), fuzzySets.desirable)
    );
  ...
  _initAssaultRifleFuzzyModule(fuzzySets) {
    ...
    fuzzyModuleAssaultRifle.addRule(
      new YUKA.FuzzyRule(
        new YUKA.FuzzyAND(fuzzySets.targetClose, lowAssault),
        fuzzySets.undesirable
      )
    );
    fuzzyModuleAssaultRifle.addRule(
      new YUKA.FuzzyRule(new YUKA.FuzzyAND(fuzzySets.targetClose, okayAssault), fuzzySets.desirable)
    );
    fuzzyModuleAssaultRifle.addRule(
      new YUKA.FuzzyRule(
        new YUKA.FuzzyAND(fuzzySets.targetClose, loadsAssault),
        fuzzySets.desirable
      )
    );

    fuzzyModuleAssaultRifle.addRule(
      new YUKA.FuzzyRule(new YUKA.FuzzyAND(fuzzySets.targetMedium, lowAssault), fuzzySets.desirable)
    );
    fuzzyModuleAssaultRifle.addRule(
      new YUKA.FuzzyRule(
        new YUKA.FuzzyAND(fuzzySets.targetMedium, okayAssault),
        fuzzySets.veryDesirable
      )
    );
    fuzzyModuleAssaultRifle.addRule(
      new YUKA.FuzzyRule(
        new YUKA.FuzzyAND(fuzzySets.targetMedium, loadsAssault),
        fuzzySets.veryDesirable
      )
    );

    fuzzyModuleAssaultRifle.addRule(
      new YUKA.FuzzyRule(new YUKA.FuzzyAND(fuzzySets.targetFar, lowAssault), fuzzySets.desirable)
    );
    fuzzyModuleAssaultRifle.addRule(
      new YUKA.FuzzyRule(
        new YUKA.FuzzyAND(fuzzySets.targetFar, okayAssault),
        fuzzySets.veryDesirable
      )
    );
    fuzzyModuleAssaultRifle.addRule(
      new YUKA.FuzzyRule(
        new YUKA.FuzzyAND(fuzzySets.targetFar, loadsAssault),
        fuzzySets.veryDesirable
      )
    );
