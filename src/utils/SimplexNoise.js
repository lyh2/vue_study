...
export  default class SimplexNoise {
    ...
    config(){
        ...
        const [contributions2D, lookup2D] = this.createContributionArray(types._2D);
        const [contributions3D, lookup3D] = this.createContributionArray(types._3D);
        const [contributions4D, lookup4D] = this.createContributionArray(types._4D);
        const perm = new Uint8Array(256);
        const perm2D = new Uint8Array(256);
        const perm3D = new Uint8Array(256);
        const perm4D = new Uint8Array(256);
        const source = new Uint8Array(setOf(256, i => i));
        var seed = this.shuffleSeed(this.clientSeed, 3);
        doFor(256, i => {
            i = 255 - i;
            seed = this.shuffleSeed(seed);
            var r = (seed + 31) % (i + 1);
            r += r < 0 ? i + 1 : 0;
            perm[i] = source[r];
	...
	createContribution(type, baseSet, index) {
		var i = 0;
	...
	createLookupPairs(lookupArray, contributions){
		var i;
		const a = lookupArray();
		const res = new Map();
