...
import {
  ...
  RandomCombatPattern,
  SpreadCombatPattern,
} from '../patterns/CombatPatterns';
import {
  CircleMovementPattern,
  LeftRightMovementPattern,
  PursuitBehaviorMovementPattern,
  WavyMovementPattern,
} from '../patterns/MovementPatterns';
import { CustomAnimationProperty } from './AnimationSystem';
...
export default class StageManager {
  ...
  _loadStage03() {
    const world = this.world;
    world.updateField(15, 1, 15);
    world.controls.setPosition(0, 0.5, 0);
    world.controls.resetRotation();
    // enemies
    const guard = world._createGuard();
    guard.position.set(0, 0.5, 0);
  ...
  _loadStage07() {
    ...
    for (let i = 0; i < pursuerCount; i++) {
      const pursuer = world._createGuard();
  ...
  _loadStage08() {
    const world = this.world;
    world.guardsProtected = true;
    world.updateFiled(15, 1, 15);
    world.controls.setPosition(0, 0.5, 0);
    world.controls.resetRotation();

    const guard = world._createGuard();
    guard.position.set(0, 0.5, -8);
    const combatPattern = new FocusCombatPattern();
  ...
  _loadStage10() {
    ...
    // 创建动画
    const animation = new CustomAnimationProperty();
  ...
  _loadStage11() {
    ...
    let pursuer = this.createPursuer(world);
    pursuer.position.set(0, 0.5, 11);
    world.addPursuer(pursuer);

    pursuer = this.createPursuer(world);
  ...
  _loadStage13() {
    ...
    for (let i = 0; i < pursuerCount; i++) {
      const pursuer = this.createPursuer(world);
