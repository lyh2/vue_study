...
class MovementPattern extends YUKA.State {
  constructor() {
    super();
    this.speed = 1.5;
    this.spread = 4;
...
class CircleMovementPattern extends MovementPattern {
  ...
  execute(enemy) {
    const t = enemy.world.time.getElapsed() * this.spread;
...
class PursuitBehaviorMovementPattern extends MovementPattern {
  ...
  enter(enemy) {
    const pursuitBehavior = new YUKA.PursuitBehavior(enemy.world.player);
